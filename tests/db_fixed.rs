...
fn convert_to_fixed_bytes(addr: Address) -> MyFixedBytes {
    MyFixedBytes(FixedBytes::<32>::left_padding_from(addr.as_slice()))
}
...
async fn test_sqlite_basic_operations() {
    ...
    sqlx::query(
        "CREATE TABLE test_fixed (
            ...
            fixed_bytes BINARY(32) NOT NULL,
            name TEXT
        )"
    )
    .execute(&pool)
...
async fn test_postgres_zero_and_special_fixed() {
    ...
    let special_fixed = [
        (MyFixedBytes(FixedBytes::<32>::ZERO), "Zero fixed bytes"),
        (MyFixedBytes(FixedBytes::<32>::new([15u8;32])), "Max fixed bytes"),
        (MyFixedBytes(FixedBytes::<32>::right_padding_from(address!("0xdead000000000000000000000000000000000000").as_slice())), "Dead fixed bytes"),
    ];
    ...
    async fn test_postgres_advanced_queries() {
        ...
        assert_eq!(user1_hash.to_string(), "0x000000000000000000000000742D35CC6635C0532925A3b8D42cC72b5c2A9a1D".to_lowercase());
        assert_eq!(user3_hash.to_string(), "0x000000000000000000000000dEad000000000000000000000000000000000000".to_lowercase());
